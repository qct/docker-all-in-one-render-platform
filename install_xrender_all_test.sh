...
docker_image_run_command["portal"]="docker run -itd --privileged --name xrender_nginx --net=host -v $DOCKER_DATA_BASE/portal:/root/nginx/portal $DOCKER_SH_MOUNT ${docker_image_tag[portal]}"
docker_image_run_command["rabbitmq"]="docker run -d --privileged --name xrender_rabbitmq --net=host -e RABBITMQ_USERNAME=xrender -e RABBITMQ_PASSWORD=xrender $DOCKER_SH_MOUNT ${docker_image_tag[rabbitmq]}"
docker_image_run_command["condor"]="docker run -itd --privileged --name xrender_condor --net=host $DOCKER_SH_MOUNT ${docker_image_tag[condor]}"
...
docker_image_run_command["msp"]="docker run -itd --privileged --name xrender_msp --net=host $DOCKER_SH_MOUNT ${docker_image_tag[platform]}"
docker_image_run_command["python"]="docker run -itd --privileged --net=host  --name xrender_python -e LANG=C.UTF-8 $DOCKER_SH_MOUNT ${docker_image_tag[python]}"
docker_image_run_command["redis"]="docker run --name xrender_redis -d --restart=always  --net=host  $DOCKER_SH_MOUNT ${docker_image_tag[redis]}"
docker_image_run_command["ftp"]="docker run -itd --privileged --net=host --name xrender_ftp -v $DOCKER_DATA_BASE/ftp:/home/ftp1 $DOCKER_SH_MOUNT ${docker_image_tag[ftp]}"
docker_image_run_command["mysql"]="docker run -itd --privileged --net=host --name xrender_mysql $DOCKER_SH_MOUNT ${docker_image_tag[mysql]}"
